...
class ProdsDir extends ProdsPath
{
 ...
 /**
  * chmod the directory on server
  * @param string $dirname directory name
  * @param string $permission access level (null|read|write|own)
  * @param string $user user/group to grant permission to
  * @param int $recursive recursive chmod
  */
  public function chmod($dirname="", $permission="", $user="", $recursive)
  {
    $conn = RODSConnManager::getConn($this->account);
    $conn->modifyAccessControl($dirname, $permission, $user, $recursive);
  ...
  * Get children directories of this dir.
	...
	*/
  /*
  public function getChildDirs($terms, array $orderby=array(), $startingInx=0,
    $maxresults=-1, &$total_num_rows=-1)
  {
    //$terms=array("descendantOnly"=>true,"recursive"=>false,"metadata"=>$meta);    
 ...
 /**
	...
	*/
  /*
  public function getChildFiles($terms, array $orderby=array(), $startingInx=0,
 ...
 /**
  * TODO: modify the terms <- this already supports searching based on metadata
  *
  ...
  * TODO: modify the terms <- this already supports searching based on metadata
  *
  * query metadata, and find matching directories.
  * @param array $terms an assositive array of search conditions, supported ones are:
  * -    'name' (string) - partial name of the target (file or dir)
  * -    'descendantOnly' (boolean) - whether to search among this directory's decendents. default is false.
  * -    'recursive'      (boolean) - whether to search recursively, among all decendents and their children. default is false. This option only works when 'descendantOnly' is true
    ...
    foreach($terms as $term_key => $term_val)
    {
      switch ($term_key)
      {
        ...
        case 'metadata':
          ...
          {
            //$condition->add('COL_META_DATA_ATTR_NAME', '=', $meta->name);
            $condition->add('COL_META_COLL_ATTR_NAME', '=', $meta->name); // 08/04/2009 [fixed: check for directory metadata]
            if (isset($meta->op))
              $op=$meta->op;
            else
              $op='=';
            if ($op=='like')
              ...
              $value=$meta->value;
            //$condition->add('COL_META_DATA_ATTR_VALUE', $op, $value);
            $condition->add('COL_META_COLL_ATTR_VALUE', $op, $value); // 08/04/2009 [fixed: check for directory metadata]
          }
          break;
