...
class ProdsQuery
{
  ...
  /**
	* Get all existing groups on the server.
	* Implements: select user_name from r_user_main where user_type_name='rodsgroup'
	* @return array of strings (group names). 
	*/
  public function getAllExistingGroups()
  {
    $flds=array("COL_USER_NAME"=>NULL); 
    $select=new RODSGenQueSelFlds(array_keys($flds), array_values($flds));
    $condition=new RODSGenQueConds();
    $condition->add('COL_USER_TYPE', '=', 'rodsgroup');
    ...
    else
    {
      ...
      return $values['COL_USER_NAME'];
    }
  }
 
  /**
	* Get all members of a group on the server.
	* Implements: 	    
	    select user_name 
	    from r_user_main, r_user_group
	    where r_user_group.user_id=r_user_main.user_id and r_user_group.group_user_id =	select user_id
	                                                                                    from r_user_main
	                                                                                    where user_name=group
	* @return array of strings (user names). 
	*/
  public function getGroupMembers($group)
  {
    ...
    $select=new RODSGenQueSelFlds(array_keys($flds), array_values($flds));
    $condition=new RODSGenQueConds();
    $condition->add('COL_USER_GROUP_NAME', '=', $group);
    $conn = RODSConnManager::getConn($this->account);
    $results=$conn->query($select, $condition);
    RODSConnManager::releaseConn($conn);        
    
    // use the results for the 2nd query    
    if ($results->getNumRow() < 1)
      return array();
    else
    {
