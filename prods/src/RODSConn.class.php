...
class RODSConn 
{
   ...
   * @param string $auth_scheme auth scheme 
   */
  ...
  public function connect()
  {
    ...
    if ($pass == NULL)
    {
        $msg=RODSMessage::packSvrConnectMsg($user, $zone);
    }
    else 
    {
    ...
    if ($pass == NULL)
    {
        $pad_pass=str_pad(RODS_SVR_ADMIN_PASSWORD, MAX_PASSWORD_LEN,"\0"); // how to customize this
    }
    else
    {
    ...
    if ($pass == NULL)
    {
        $resp_packet=new RP_authResponseInp($response,"admin");
    }
    else
    {
  ...
  * @param string filename, permission, user, recursive
  */
  public function modifyAccessControl ($filename, $permission, $user, $recursive)
  {
    $chmod_pk = new RP_modACLInp($filename, $permission, $user, $recursive);
    $msg = new RODSMessage("RODS_API_REQ_T", $chmod_pk, $GLOBALS['PRODS_API_NUMS']['MOD_ACCESS_CONTROL_AN']); 
    fwrite($this->conn, $msg->pack()); // send it
    $msg = new RODSMessage();
    $intInfo = (int) $msg->unpack($this->conn); 
    if ($intInfo<0)
    {
        // error has occured
        throw new RODSException("RODSConn::modifyAccessControl has got an error from the server",
          $GLOBALS['PRODS_ERR_CODES_REV']["$intInfo"]);
    }
  }
  ...
  */
  ...
  /**
  * Runs iadmin commands on RODS server
  * @param string $command icommand.
  ...
  *
  * @param string $admin_password admin password (for modifying user password).
  * @param string $arguments arguments for the commands.
  */  
  
  public function iadmin ($command="", $admin_password="")
  {
    $args = array();
    ...
    {
      case "mkuser":
        $this->generalAdmin("add", "user", $args[0], "rodsuser", "", "", "", "", "", "");
        break;
      case "moduser":
        if ($args[1] == "password") {
          // needs to obfuscate if changing password
          $obf_pass = obf_password($admin_password, $args[2]);
          $this->generalAdmin("modify", "user", $args[0], $args[1], $obf_pass, "", "", "", "", "");
        } else {
          $this->generalAdmin("modify", "user", $args[0], $args[1], $args[2], "", "", "", "", "");
        }
        break;                        
      case "rmuser":
        $this->generalAdmin("rm", "user", $args[0], "", "", "", "", "", "", "");
        break;
      case "mkdir":          
        $this->generalAdmin("add", "dir", $args[0], "", "", "", "", "", "", "");              
        break;
      case "rmdir":
        $this->generalAdmin("rm", "dir", $args[0], "", "", "", "", "", "", "");
        break;
      case "mkresc":
        $this->generalAdmin("add", "resource", $args[0], $args[1], $args[2], $args[3], $args[4], $args[5], "", "");
        break;
      case "modresc":                        
        $this->generalAdmin("modify", "resource", $args[0], $args[1], $args[2], "", "", "", "", "");                    
        break;                        
      case "rmresc":
        $this->generalAdmin("rm", "resource", $args[0], "", "", "", "", "", "", "");
        break;
      case "mkzone":          
        $this->generalAdmin("add", "zone", $args[0], $args[1], $args[2], $args[3], "", "", "", "");              
        break;
      case "modzone":          
        if ($args[1] == "name" && $args[0] == $this->account->zone) {
          // changing local zone
          $this->generalAdmin("modify", "localzone", $args[0], $args[2], "", "", "", "", "", "");
        } else {
          $this->generalAdmin("modify", "zone", $args[0], $args[1], $args[2], "", "", "", "", "");              
        }
        break;                        
      case "rmzone":
        $this->generalAdmin("rm", "zone", $args[0], "", "", "", "", "", "", "");        
        break;                                                                                                                                                
      case "mkgroup":  
        $this->generalAdmin("add", "user", $args[0], "rodsgroup", "", "", "", "", "", "");
        break;
      case "rmgroup":
        $this->generalAdmin("rm", "user", $args[0], "rodsgroup", "", "", "", "", "", "");
        break;
      case "atg":
        $this->generalAdmin("modify", "group", $args[0], "add", $args[1], "", "", "", "", "");
        break;
      case "rfg":
        $this->generalAdmin("modify", "group", $args[0], "remove", $args[1], "", "", "", "", "");
        break;
      case "atrg":                        
        $this->generalAdmin("modify", "resourcegroup", $args[0], "add", $args[1], "", "", "", "", "");
        break;
      case "rfrg":
        $this->generalAdmin("modify", "resourcegroup", $args[0], "remove", $args[1], "", "", "", "", "");
        break;                                     
      case "at":
        $this->generalAdmin("add", "token", $args[0], $args[1], $args[2], $args[3], $args[4], $args[5], "", "");                        
        break;
      case "rt":     
        $this->generalAdmin("rm", "token", $args[0], $args[1], "", "", "", "", "", "");                   
        break;                        
      case "pv":
        $this->generalAdmin("pvacuum", $args[0], $args[1], "", "", "", "", "", "");
        break;
      default:
        throw new Exception("The ".$command." command is not available yet.");
        break;                            
    }
  }

 /**
  * General admin query command for executing icommands
  * @param string arg0 - arg9
  */
  public function generalAdmin ($arg0="", $arg1="", $arg2="", $arg3="", $arg4="", $arg5="", $arg6="", $arg7="", $arg8="", $arg9="")
  {
    $genAdmin_pk = new RP_genAdminInp($arg0, $arg1, $arg2, $arg3, $arg4, $arg5, $arg6, $arg7, $arg8, $arg9);
    $msg = new RODSMessage("RODS_API_REQ_T", $genAdmin_pk, $GLOBALS['PRODS_API_NUMS']['GENERAL_ADMIN_AN']); 
    fwrite($this->conn, $msg->pack()); // send it
    $msg = new RODSMessage();
    $intInfo = (int) $msg->unpack($this->conn); 
    if ($intInfo<0)
    {
        // error has occured
        throw new RODSException("RODSConn::generalAdmin has got an error from the server",
          $GLOBALS['PRODS_ERR_CODES_REV']["$intInfo"]);
    }
  }  
